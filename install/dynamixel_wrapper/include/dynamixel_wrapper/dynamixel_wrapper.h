...
public:
    ...
    void setHomingOffset(double degree) { write(motor_config_.homing_offset, static_cast<int>(degree / 360.0f * motor_config_.resolution)); }
    void setMovingThreshold(float rpm) { write(motor_config_.moving_threshold, static_cast<int>(rpm / motor_config_.velocity_scaling_factor)); }
    void setMaxVoltageLimit(float voltage) { write(motor_config_.max_voltage_limit, static_cast<int>(voltage / motor_config_.voltage_scaling_factor)); }
    void setminVoltageLimit(float voltage) { write(motor_config_.min_position_limit, static_cast<int>(voltage / motor_config_.voltage_scaling_factor)); }
    void setCurrentLimit(double mA) { write(motor_config_.current_limit, static_cast<int>(mA / motor_config_.current_scaling_factor)); }
    void setAccelerationLimit(float rpm2) { write(motor_config_.acceleration_limit, static_cast<int>(rpm2 / motor_config_.accleration_scaling_factor)); }
    void setVelocityLimit(double rpm) { write(motor_config_.velocity_limit, static_cast<int>(rpm / motor_config_.velocity_scaling_factor)); }
    void setMaxPositionLimit(double degree) { write(motor_config_.max_position_limit, static_cast<int>(degree / 360.0f * motor_config_.resolution)); }
    void setMinPositionLimit(double degree) { write(motor_config_.min_position_limit, static_cast<int>(degree / 360.0f * motor_config_.resolution)); }
    void setTorqueEnable(bool state) { write(motor_config_.torque_enable, static_cast<int>(state)); }
    ...
    std::string getHardwareErrorStatus() {
        ...
        for (int i = 0; i < 8; i++) {
            if ((errorNum >> i) & 0x01)
                errorCode += (motor_config_.errorCode[i] + " ");
        }
        if (errorCode == "")
            errorCode += "No Error";
        return errorCode;
    ...
    void setGoalCurrent(double mA) { write(motor_config_.goal_current, static_cast<int>(mA / motor_config_.current_scaling_factor)); }
    void setGoalVelocity(double rpm) { write(motor_config_.goal_velocity, static_cast<int>(rpm / motor_config_.velocity_scaling_factor)); }
    void setProfileAcceleration(double rpm2) { write(motor_config_.profile_acceleration, static_cast<int>(rpm2 / motor_config_.accleration_scaling_factor)); }
    void setProfileVelocity(double rpm) { write(motor_config_.profile_velocity, static_cast<int>(rpm / motor_config_.velocity_scaling_factor)); }
    void setGoalPosition(double degree) { write(motor_config_.goal_position, static_cast<int>(degree / 360.0f * motor_config_.resolution)); }
...
private:
    ...
    rclcpp::Logger logger_ = rclcpp::get_logger("dynamixel_wrapper");
};
...
void dynamixel_wrapper::write(dynamixel_wrapper_config_item item, int value) {
    ...
    if (byte_size == 1) {
        ...
        RCLCPP_ERROR(logger_, "Byte size is undefined");
        return;
    }

    if (!dxl_comm_result) {
        RCLCPP_ERROR(logger_, "Failed to write to Dynamixel ID %d", id_);
    }
}
...
int32_t dynamixel_wrapper::read_signed(dynamixel_wrapper_config_item item) {
    ...
    if (byte_size == 1) {
        uint8_t value8;
        dxl_base_->packetHandler->read1ByteTxRx(dxl_base_->portHandler, id_, address, &value8);
        result = static_cast<int8_t>(value8);
    } else if (byte_size == 2) {
        uint16_t value16;
        dxl_base_->packetHandler->read2ByteTxRx(dxl_base_->portHandler, id_, address, &value16);
        result = static_cast<int16_t>(value16);
    } else if (byte_size == 4) {
        uint32_t value32;
        dxl_base_->packetHandler->read4ByteTxRx(dxl_base_->portHandler, id_, address, &value32);
        result = static_cast<int32_t>(value32);
    } else {
        RCLCPP_ERROR(logger_, "Byte size is undefined");
        return 0;
    }
